...
- O tema claro/escuro e uma preferencia local por usuario, persistida como `rocketserviceDarkMode:<userId>`. Contas diferentes no mesmo navegador nao compartilham a escolha. A rota `/login` sempre usa o tema claro; uma preferencia escura salva so e aplicada depois da autenticacao daquele usuario.
- O `toggleTheme` depende do identificador do usuario autenticado e deve fazer parte das dependencias do valor memoizado do contexto. Sem isso, o callback criado antes da autenticacao permanece sem uma chave de usuario e o seletor ignora o clique.
- A rolagem do menu lateral e estilizada no proprio `Drawer`: trilha na cor da sidebar e indicador estreito de baixo contraste. Nao deve aparecer uma faixa branca entre a sidebar e o conteudo, inclusive no modo escuro.
