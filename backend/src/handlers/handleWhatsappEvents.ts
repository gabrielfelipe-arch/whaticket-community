...
const handoffToHuman = async (
  ...
  if (Number(ticket.queueId || 0) !== Number(humanHandoffQueueId)) {
    await ticket.update({ queuePositionMessageSentAt: null });
  }
...
const buildQualificationAiOpeningMessage = (
  ...
  const toHumanNeed = (value = ""): string | null => {
    ...
    if (normalized.includes("1 encontro")) return "um encontro pontual";
    if (normalized.includes("mais de 1 encontro")) return "uma atividade em mais de um encontro";
    if (normalized.includes("reuniao")) return "uma reunião com sua equipe";
    if (normalized.includes("processo seletivo")) return "um processo seletivo";
    if (normalized.includes("pos-graduacao") || normalized.includes("pos graduacao")) return "uma pós-graduação";
    if (normalized.includes("professor particular")) return "aulas particulares";
    if (normalized.includes("mais informacoes") || normalized.includes("nao ofereci")) return "entender melhor as possibilidades antes de decidir";

    return text.charAt(0).toLowerCase() + text.slice(1);
  };

  const toHumanMoment = (value = ""): string | null => {
    const text = normalizeAnswer(value);
    const normalized = text
      .toLowerCase()
      .normalize("NFD")
    ...
    if (!normalized) return null;
    if (normalized.includes("ja aluguei")) return "você já tem experiência alugando espaço";
    if (normalized.includes("nunca aluguei")) return "você já fez esse tipo de encontro, mas ainda não alugou uma sala";
    if (normalized.includes("primeira turma") || normalized.includes("primeiro encontro")) return "você está organizando isso pela primeira vez";
    if (normalized.includes("pesquisando")) return "você ainda está pesquisando e comparando opções";

    return text.charAt(0).toLowerCase() + text.slice(1);
  };

  const collectedAnswers = String(summary || "")
    .split(/\r?\n/)
    .map(line => line.trim())
    .filter(line => line.startsWith("- "))
    .slice(0, 3);
  const mainNeed = toHumanNeed(collectedAnswers[0]);
  const experienceMoment = toHumanMoment(collectedAnswers[1]);
  const contextLine = [
    mainNeed ? `Entendi que você precisa de apoio para ${mainNeed}.` : "Entendi suas respostas iniciais.",
    experienceMoment ? `E pelo que você marcou, ${experienceMoment}.` : ""
  ].filter(Boolean).join(" ");

  return [
    `Perfeito${contactFirstName ? `, ${contactFirstName}` : ""}. Sou a ${aiName}, assistente da ${company}.`,
    contextLine,
    "Vou seguir por esse caminho com você. Para começar, quantas pessoas vão participar?"
  ].join("\n\n");
};
    ...
    return parsed
      .map((item, index) => ({
        ...
        nextMessages: Array.isArray(item?.nextMessages)
          ? item.nextMessages.map((message: any) => ({
            ...
            })).filter((message: any) => message.body || message.mediaUrl)
          : [],
        nextQuestionId: item?.nextQuestionId ? Number(item.nextQuestionId) : null,
        targetQueueId: item?.targetQueueId ? Number(item.targetQueueId) : null,
        uraOptionId: item?.uraOptionId ? Number(item.uraOptionId) : null
      }))
...
const fillDefaultGlpiLocationIfNeeded = async (
  ...
  if (question.type !== "glpi_location" && question.glpiField !== "location") return false;
  ...
  await QualificationFormAnswer.create({
    ...
    value: String(location.glpiId),
    rawValue: "localizacao_padrao",
    optionLabel: location.name || location.completeName || String(location.glpiId),
...
const getQualificationRouteOption = (
  ...
  return selected.find(option => {
    const action = String(option.nextAction || "").trim();
    const message = String(option.nextMessage || "").trim();
    const messages = option.nextMessages || [];
    return message || messages.length || (action && action !== "NEXT");
  }) || null;
};

const sortQualificationOptionsByLabel = (options: QualificationOption[]): QualificationOption[] =>
  [...options].sort((a, b) =>
...
const formatQualificationAnswerFieldLabel = (answer: QualificationFormAnswer): string => {
  ...
  return String(answer.label || "Resposta").replace(/[?:]+$/g, "").trim() || "Resposta";
};
...
const sendQualificationRouteMessage = async (
  ...
  ticket: Ticket,
  contactPayload: ContactPayload,
  routeOption: QualificationOption
): Promise<void> => {
  const messages = [
    ...((routeOption.nextMessages || []).filter(message => message.body || message.mediaUrl)),
    ...(
      String(routeOption.nextMessage || "").trim() && !(routeOption.nextMessages || []).length
        ? [{ body: String(routeOption.nextMessage || "").trim() }]
        : []
    )
  ];
...
const executeQualificationRoute = async (
  whatsappId: number,
  ticket: Ticket,
  contactPayload: ContactPayload,
  flow: any,
  ...
  const action = String(routeOption.nextAction || "NEXT").trim();

  await sendQualificationRouteMessage(whatsappId, ticket, contactPayload, routeOption);

  if (action === "NEXT") {
    return false;
  }

  if (action === "GOTO_QUESTION" && routeOption.nextQuestionId) {
    const nextQuestion = questions.find(item => Number(item.id) === Number(routeOption.nextQuestionId));
    if (nextQuestion) {
  ...
  if (action === "OPEN_URA_OPTION" && routeOption.uraOptionId) {
    ...
    const submenuOptions = getUraOptionsByParent(flow, Number(targetOption.id));
    const submenu = buildUraMenu(flow, targetOption);
    if (submenuOptions.length && submenu) {
      await sendConfiguredMessage({
        ...
        body: submenu,
        ticket,
        mediaUrl: targetOption.responseMediaUrl,
        mediaType: targetOption.responseMediaType,
        mediaName: targetOption.responseMediaName
      });
      await ticket.update({
        currentUraOptionId: targetOption.id,
        uraMenuSentAt: new Date(),
        uraInvalidAttempts: 0,
        uraActive: true,
        aiConversationSummary: summary || ticket.aiConversationSummary || null,
    ...
    if (targetOption.responseMessage || targetOption.responseMediaUrl) {
      await sendConfiguredMessage({
        whatsappId,
        contactPayload,
        body: targetOption.action === "SEND_MESSAGE"
          ? appendSubmenuNavigationFooter(targetOption.responseMessage, targetOption)
          : targetOption.responseMessage,
        ticket,
        mediaUrl: targetOption.responseMediaUrl,
        mediaType: targetOption.responseMediaType,
        mediaName: targetOption.responseMediaName
    ...
    if (targetOption.action === "SEND_MESSAGE") {
      await ticket.update({
        currentUraOptionId: targetOption.parentOptionId || null,
        uraMenuSentAt: new Date(),
        uraInvalidAttempts: 0,
        uraActive: true,
        aiConversationSummary: summary || ticket.aiConversationSummary || null,
  ...
  if (["END_FORM", "START_AI", "TRANSFER_QUEUE", "HUMAN", "CLOSE_TICKET"].includes(action)) {
    ...
    const effectiveOption = {
      ...
      targetQueueId: routeOption.targetQueueId || selectedOption.targetQueueId || null
    } as UraOption;

    logger.info(
      { ticketId: ticket.id, responseId: response.id, routeAction: action },
      "[QUALIFICATION FORM] Completed by conditional route"
    );
...
const handleActiveQualificationForm = async (
  ...
  if (["glpi_entity", "glpi_location"].includes(question.type) && String(messageBody || "").trim() === "0") {
    ...
    if (previousQuestion) {
      const questionIdsToClear = questions
        .slice(currentIndex - 1)
        .map(item => item.id);
      await QualificationFormAnswer.destroy({
        where: {
          responseId: response.id,
          questionId: { [Op.in]: questionIdsToClear }
        }
      });
      await response.update({ currentQuestionId: previousQuestion.id, invalidAttempts: 0 });
      await sendTextMessage(
        whatsappId,
        contactPayload,
        await buildQualificationQuestionMessage(previousQuestion, selectedOption.allowQualificationFormSkip, response),
        ticket,
        "ura"
      );
      return true;
  ...
  const questions = await getQualificationQuestions(response.formId);
  const routeOption = answer.ok ? getQualificationRouteOption(answer) : null;

  if (routeOption && await executeQualificationRoute(
    whatsappId,
    ticket,
    contactPayload,
    flow,
